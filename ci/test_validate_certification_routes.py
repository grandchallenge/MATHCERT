...
class CertificationRouteTests(unittest.TestCase):
 ...
 def test_h_cannot_return_to_submitted(self):
  d=self.load_registry();r=next(r for r in d["routes"] if r["campaign_id"]=="OTP-H-GAPCVP");r["intake_status"]="submitted";r["cert_output"]=None;self.assertTrue(any("governed intake state drift" in x for x in self.errors(d)))
 def test_h_output_pointer_drift_fails(self):
  d=self.load_registry();r=next(r for r in d["routes"] if r["campaign_id"]=="OTP-H-GAPCVP");r["cert_output"]["digest"]="0"*40;self.assertTrue(any("output identity drift" in x for x in self.errors(d)))
 ...
 def test_b1_cannot_return_to_submitted(self):
  d=self.load_registry();r=next(r for r in d["routes"] if r["campaign_id"]=="OTP-B1-BINARY-CODES");r["intake_status"]="submitted";r["cert_output"]=None;self.assertTrue(any("governed intake state drift" in x for x in self.errors(d)))
 def test_b1_output_pointer_drift_fails(self):
  d=self.load_registry();r=next(r for r in d["routes"] if r["campaign_id"]=="OTP-B1-BINARY-CODES");r["cert_output"]["digest"]="0"*40;self.assertTrue(any("output identity drift" in x for x in self.errors(d)))
 ...
 def test_i_ramsey_output_pointer_drift_fails(self):
  d=self.load_registry();next(r for r in d["routes"] if r["campaign_id"]=="OTP-I-RAMSEY")["cert_output"]["digest"]="0"*40;self.assertTrue(any("output identity drift" in x for x in self.errors(d)))
 def test_g_quantum_output_pointer_drift_fails(self):
  d=self.load_registry();next(r for r in d["routes"] if r["campaign_id"]=="OTP-G-QUANTUM-PARALLEL-REPETITION")["cert_output"]["digest"]="0"*40;self.assertTrue(any("output identity drift" in x for x in self.errors(d)))
 def test_g_quantum_packet_drift_fails(self):

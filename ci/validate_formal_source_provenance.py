#!/usr/bin/env python3
"""Validate expanded Formal Conjectures provenance and Cert intake synchronization."""
...
SCOPES = ["zeta3","zeta5","zeta7","zeta9","zeta11","universal-odd-irrationality","odd-value-infinitude","finite-zudilin-disjunction"]

def load_json(path: Path) -> Any:
    return json.loads(path.read_text(encoding="utf-8"))

def provenance_errors(record_path: Path=RECORD_PATH, routes_path: Path=ROUTES_PATH) -> list[str]:
    r=load_json(record_path); routes=load_json(routes_path); e=[]
    if r.get("schema_version")!="1.0.0" or r.get("verification_id")!="MC-FC-GOV-001":
    ...
    if lin!=expected_lin: e.append("lineage drift")
    replay=r.get("independent_replay",{})
    exp={
      "workflow_run_id":30544600547,"artifact_id":8761186970,
      "artifact_downloaded":True,"archive_byte_length":73686,
      ...
      "lean_toolchain":"leanprover/lean4:v4.27.0","mathlib_revision":"v4.27.0"}
    for k,v in exp.items():
        if replay.get(k)!=v: e.append(f"replay {k} drift")
    ...
    if set(members)!=set(expected_members): e.append("replay member set drift")
    for n,(size,dig,sc,pc) in expected_members.items():
        m=members.get(n,{})
        if (m.get("byte_length"),m.get("sha256"),m.get("statement_count"),m.get("problem_count"))!=(size,dig,sc,pc):
            e.append(f"replay member drift: {n}")
    if r.get("common_provider_blobs")!=COMMON: e.append("common provider artifacts drift")
    campaigns=r.get("campaign_provider_blobs",{})
    if campaigns != CAMPAIGN_BLOBS:
        e.append("campaign provider artifacts drift")
    if len(campaigns.get("OZ-001",{})) != 8:
        e.append("odd-zeta theorem lattice incomplete")
    if r.get("solve_manifest_blobs")!=MANIFESTS: e.append("Solve manifest identities drift")
    packet_map=r.get("solve_handoff_packets",{})
    if set(packet_map)!=set(PACKETS): e.append("Solve packet coverage drift")
    for cid,dig in PACKETS.items():
        state="ready" if cid in READY else "pending"
        if packet_map.get(cid)!={"blob":dig,"state":state}: e.append(f"{cid} packet drift")
    ctl=r.get("scope_controls",{})
    if ctl.get("required_odd_zeta_scopes")!=SCOPES: e.append("odd-zeta scope drift")
    if ctl.get("explicit_non_routes")!={"BSD-001":"adjacency-only","HC-001":"bounded-negative-source-screen","YM-001":"lexical-false-positive"}:
        e.append("non-route drift")
    if ctl.get("pilot_lane")!={"source_id":"FC-GDM-001","campaigns":["RH-001","NS-CI-001"],"status":"unchanged"}:
        e.append("RH/NS pilot drift")
    result=r.get("certification_result",{})
    if result!={"source_identity_verified":True,"extraction_archive_replayed":True,
      "route_manifest_identity_verified":True,"handoff_packet_identity_verified":True,
      "target_declaration_elaborated":False,"concordance_theorem_kernel_checked":False,
      "mathematical_target_proved":False,"adjudicated_outputs":0}:
        e.append("certification boundary drift")
    route_map={x.get("campaign_id"):x for x in routes.get("routes",[]) if isinstance(x,dict)}
    if set(route_map)!=set(MANIFESTS): e.append("route coverage drift")
    for cid in MANIFESTS:
        x=route_map.get(cid,{})
        src=x.get("source_manifest",{})
        if src.get("commit_sha")!=SOLVE or src.get("digest")!=MANIFESTS[cid]: e.append(f"{cid} route manifest drift")
        state="ready" if cid in READY else "pending"
        if x.get("intake_status")!=state: e.append(f"{cid} route state drift")
        if cid in READY:
            pkt=x.get("intake_packet",{})
            if pkt.get("commit_sha")!=SOLVE or pkt.get("digest")!=PACKETS[cid]: e.append(f"{cid} accepted packet drift")
        elif x.get("intake_packet") is not None: e.append(f"{cid} pending packet promoted")
        if x.get("cert_output") is not None: e.append(f"{cid} adjudication inflation")
    return e
...
def main()->int:
    ...
    print("verified FC-GDM-002 replay, provider artifacts, eight Solve manifests and packets, three ready intakes, five pending routes, and zero adjudications")
    return 0

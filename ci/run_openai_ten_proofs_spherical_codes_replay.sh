...
assert wp['target_scope']['lean_theorems']==t; assert wp['execution_contract']['deterministic_commands']==['lake exe cache get','lake build MetricCodes','lake exe comparator ComparatorChallenges/B_SphericalCodes.json']; assert wp['execution_contract']['permitted_axioms']==['propext','Classical.choice','Quot.sound']; assert wp['execution_contract']['challenge_sorry_warning_count']==4; assert cfg['theorem_names']==t and cfg['permitted_axioms']==['propext','Classical.choice','Quot.sound'] and cfg['enable_nanoda'] is True
assert not any(r.get('campaign_id')=='OTP-B2-SPHERICAL-CODES' for r in routes['routes'])
qs=wp['target_scope']['mandatory_qualifications']; assert any('predecessor seven-target spherical authority transfers nothing' in q.lower() for q in qs); assert any('0.39661+o(1)' in q for q in qs)
PY

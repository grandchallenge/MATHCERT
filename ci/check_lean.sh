...
control_family() {
  local path="${1,,}"
  case "$path" in
    *otp_g_quantum_parallel_repetition*|*otp-g-quantum-parallel-repetition*|*quantum_parallel_repetition_replay*) echo "OTP-G-QUANTUM-PARALLEL-REPETITION" ;;
...
python3 ci/validate_external_catalog_certification_intake.py
python3 -m unittest ci/test_external_catalog_certification_intake.py -v
...
if [[ -f ci/validate_otp_g_quantum_parallel_repetition_certification.py ]]; then

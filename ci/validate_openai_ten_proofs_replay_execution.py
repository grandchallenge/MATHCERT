...
def validation_errors(
    ...
    if not isinstance(record, dict) or set(record) != expected_keys:
        errors.append("OTP-CERT-REPLAY-001: top-level shape drift")
        return errors
    ...
    if workflow != {
        "path": ".github/workflows/otp-family-replay.yml",
        "clean_room_matrix": True,
        "toolchain": EXPECTED_TOOLCHAIN,
    }:
        errors.append("OTP-CERT-REPLAY-001: workflow or toolchain identity drift")
    ...
    if record.get("families") != EXPECTED_FAMILIES:
    ...
    if record.get("route_controls") != EXPECTED_ROUTE_CONTROLS:
    ...
    workflow_tokens = [
        "runs-on: ubuntu-24.04", "timeout-minutes: 55", "fail-fast: false",
        "MATHCERT_HEAD_SHA:", "MATHCERT_WORKFLOW_SHA:", "openai/ten-proofs",
        ...
        EXPECTED_TOOLCHAIN["lean4export"], EXPECTED_TOOLCHAIN["landrun"], EXPECTED_TOOLCHAIN["nanoda"],
        "OTP-F-EHRHART", "OTP-J1-COMPACTNESS", "OTP-J2-TWO-DEGENERATE", "if: always()",
    ]
    ...
    runner_tokens = [
        'expected_lean_version="4.32.0"', "Lean toolchain mismatch",
        EXPECTED_TOOLCHAIN["landrun"], ".lake/packages/Lean4Checker",
        "mathcert_head_sha=", "workflow_checkout_sha=", "lake exe comparator",
        "#print axioms", "Nanoda kernel accepts the solution",
        "Lean default kernel accepts the solution", "expected_comparator_boundary",
        "source_revision_drift_detected", "blocked_pending_forge_audit",
        "https://github.com/grandchallenge/MATHFORGE/issues/52",
        EXPECTED_SOURCE_REVISION["admitted_manuscript"]["sha256"],
        EXPECTED_SOURCE_REVISION["observed_manuscript"]["sha256"],
        "pending_source_revision_audit_and_exact_head_non_author_specialist_review",
        "aggregate_all_import_used", "may_adjudicate", "mathematical_target_proved",
        "-not -name 'SHA256SUMS'",
    ]
    ...
    if 'exec "$real_landrun" "${prefix[@]}" -- "$@"' not in adapter_text:
        errors.append("OTP-CERT-REPLAY-001: Landrun separator adapter drift")
...
def main() -> int:
    ...
    print(
        ...
        "canonical checker identities, separated manuscript revisions, zero evidence promotion, zero routes, "

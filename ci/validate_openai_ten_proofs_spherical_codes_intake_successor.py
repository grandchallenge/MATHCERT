...
HISTORICAL_PROTECTED_BASE = "5bddc3eb7d02638cf4fe959accfbfeade4964592"
...
def validate_repository_guards() -> None:
    ...
    if record.get("mathcert_subject", {}).get("protected_base") != HISTORICAL_PROTECTED_BASE:
        raise ValueError("spherical-codes intake historical protected-base identity drift")

    historical = load_historical_routes()
    routes = historical.get("routes")
    if not isinstance(routes, list):
        raise ValueError("historical spherical-codes intake route snapshot has invalid routes surface")
    if any(
        isinstance(route, dict)
        and (route.get("route_id") == OWN_ROUTE_ID or route.get("campaign_id") == FAMILY_ID)
        for route in routes
    ):
        raise ValueError("spherical-codes route authority was present in the exact historical intake snapshot")
...
def main() -> None:
    ...
    print(
        "OTP-B2-SPHERICAL-CODES successor intake validation: PASS; immutable intake route nonauthority "

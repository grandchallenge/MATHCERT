...
SPEC=importlib.util.spec_from_file_location("validator",ROOT/"ci/validate_openai_ten_proofs_non_sofic_intake_successor.py")
assert SPEC and SPEC.loader
MODULE=importlib.util.module_from_spec(SPEC); SPEC.loader.exec_module(MODULE)
class NonSoficIntakeSuccessorTests(unittest.TestCase):
    ...
    def test_target_inflation_rejected(self):
        x=copy.deepcopy(self.record); x["target_scope"]["lean_theorems"].append("SoficGroups.fake"); self.assertTrue(self.errors(x))
    def test_mathlib_pin_drift_rejected(self):
        x=copy.deepcopy(self.record); x["authority"]["official_subject"]["mathlib_finite_presentation_commit"]="0"*40; self.assertTrue(self.errors(x))

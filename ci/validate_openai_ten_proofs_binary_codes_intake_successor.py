...
RECORD = ROOT / "governance/result_family_intake_successors/OTP-B1-BINARY-CODES.json"
SCHEMA = ROOT / "schemas/openai_ten_proofs_binary_codes_result_family_intake_successor.schema.json"
...
HISTORICAL_PROTECTED_BASE = "ff9fa0a67a5a809f3519e0059f2ef9b082b1febb"
FAMILY_ID = "OTP-B1-BINARY-CODES"
OWN_ROUTE_ID = "MC-ROUTE-OTP-B1-BINARY-CODES"
...
def validate_repository_guards() -> None:
    ...
    if (LEGACY_DIR / "OTP-B1-BINARY-CODES.json").exists():
        raise ValueError("binary-codes successor was inserted into frozen historical intake namespace")

    record = load_record()
    if record.get("mathcert_subject", {}).get("protected_base") != HISTORICAL_PROTECTED_BASE:
        raise ValueError("binary-codes intake historical protected-base identity drift")

    historical = load_historical_routes()
    routes = historical.get("routes")
    if not isinstance(routes, list):
        raise ValueError("historical binary-codes intake route snapshot has invalid routes surface")
    if any(
        isinstance(route, dict)
        and (route.get("route_id") == OWN_ROUTE_ID or route.get("campaign_id") == FAMILY_ID)
        for route in routes
    ):
        raise ValueError("binary-codes route authority was present in the exact historical intake snapshot")
...
def main() -> None:
    ...
    print(
        "OTP-B1-BINARY-CODES successor intake validation: PASS; immutable intake route nonauthority "

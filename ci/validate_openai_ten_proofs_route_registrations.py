...
EHRHART_OUTPUT={"repository":"grandchallenge/MATHCERT","commit_sha":"7b79b459422951cc6e36feda34c8a6e3d615ef17","path":"certificates/formal_sources/MC-OTP-F-EHRHART-001.json","digest_algorithm":"git_blob_sha1","digest":"27a855c949b67e71372c7f0d6601d80125d33968"}
...
def validation_errors(receipt=None,routes=None,proposal_registry=None,proposal_blobs=None,routes_blob=None,proposal_registry_blob=None,current_routes_blob=None)->list[str]:
 ...
 for fam in EXPECTED_FAMILIES:
  ...
  if rec.get("intake_status")!="submitted" or rec.get("target_claim_ids")!=EXPECTED_CLAIMS[fam]:e.append(f"{fam}: protected registration state or target drift")
  if any((rec.get("cert_output") is not None,rec.get("may_adjudicate") is not False,rec.get("mathematical_target_proved") is not False,rec.get("may_promote_claim") is not False)):e.append(f"{fam}: historical registration authority inflation")
  ...
  if route.get("target_claim_ids")!=EXPECTED_CLAIMS[fam]:e.append(f"{fam}: global route target drift")
  if fam=="OTP-F-EHRHART":
   if route.get("intake_status")!="qualified" or route.get("cert_output")!=EHRHART_OUTPUT:e.append(f"{fam}: restricted successor route drift")
   if not any("not mathematically proved" in x.lower() for x in route.get("blockers",[])):e.append(f"{fam}: proof limitation blocker missing")
  else:
   if route.get("intake_status")!="submitted" or route.get("cert_output") is not None:e.append(f"{fam}: global route state inflation")
   if not any("adjudication" in x.lower() for x in route.get("blockers",[])):e.append(f"{fam}: adjudication blocker missing")
 ...
 if receipt.get("state")!=state:e.append("protected registration state inflation")

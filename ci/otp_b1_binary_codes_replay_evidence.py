...
RECORD=ROOT/"governance/result_family_replay_evidence_successors/OTP-B1-BINARY-CODES.json"
SCHEMA=ROOT/"schemas/openai_ten_proofs_binary_codes_replay_evidence.schema.json"
ROUTES=ROOT/"governance/certification_routes.json"
BUNDLE=ROOT/"evidence/openai_ten_proofs_successors/b1-binary-codes.tar.gz.b64"
WP=ROOT/"governance/result_family_work_package_successors/OTP-B1-BINARY-CODES-CERT-WP-001.json"
EXPECTED_WP_BLOB="19e1eaf5e24ce212bb020c8c40d4177ff5b4f8f9"
PRODUCER_HEAD="d39396b951fd321550e3df3995551cd53663b947"
FAMILY="OTP-B1-BINARY-CODES"
REQUIRED_MEMBERS=['environment-manifest.json', 'source-identity-report.txt', 'family-replay-log.txt', 'target-export-report.json', 'comparator-result.json', 'lean-kernel-result.json', 'nanoda-result.json', 'theorem-axiom-report.json', 'semantic-concordance-attestation.json', 'nonvacuity-attestation.json', 'minimizer-attainment-attestation.json', 'trust-boundary-scan.txt', 'independent-review-attestation.json', 'evidence-summary.json']
...
def find_family_route(node):
    if isinstance(node,dict):
        if node.get("route_id")=="MC-ROUTE-OTP-B1-BINARY-CODES" or node.get("campaign_id")==FAMILY: return True
...
def validation_errors(record=None,schema=None,bundle_text=None,check_repo=True):
    ...
    if check_repo:
        p=git("rev-parse",f"HEAD:governance/result_family_work_package_successors/OTP-B1-BINARY-CODES-CERT-WP-001.json")
...
def main():
    ...
    print("validated B1 retained replay evidence: exact producer bundle, zero route/output authority, final-head replay/review still required")
    return 0

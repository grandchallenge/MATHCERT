...
work_package_path="governance/result_family_work_package_successors/OTP-C-PERMANENT-CIRCUIT-CERT-WP01.json"
expected_work_package_blob="bdc8fba2a0677488a94efb76d2e75fb2257d0ae3"
expected_forge_commit="20a4cb716dba2586931e3eaebb079890c66044bd"
semantic_path="sources/OPENAI-TEN-PROOFS-001/semantic/OTP-C-PERMANENT-CIRCUIT/audit_record.json"
expected_semantic_blob="d47a50df90174ed03669b11b8469dc1c0788a1ea"
overlay_json_path="sources/OPENAI-TEN-PROOFS-001/target_overlays/OTP-C-PERMANENT-CIRCUIT/GCL_C_PermanentCircuit.json"
overlay_lean_path="sources/OPENAI-TEN-PROOFS-001/target_overlays/OTP-C-PERMANENT-CIRCUIT/GCL_C_PermanentCircuit.lean"
expected_overlay_json_blob="1c2aad24890425ef82f8e45fa654de32dc0e2659"
expected_overlay_lean_blob="18fc438580bab2bc003d4d3cfd9fa283da421b04"
expected_solve_commit="7d1f9edf16558ba4c4396126e24fd2c9ae4826f7"
packet_path="work_packages/OPENAI_TEN_PROOFS_WP00/result_family_handoffs/OTP-C-PERMANENT-CIRCUIT.json"
expected_packet_blob="f8443c47cee03890ca52af3e0cd39f1a54b5fc71"
...
assert_eq "$(git_blob "$forge" "$semantic_path")" "$expected_semantic_blob" "Forge semantic blob"
...
assert wp["surface_id"] == "OTP-C-PERMANENT-CIRCUIT"
...
assert sp["coefficient_field"] == "complex"
assert sp["model"] == "division_free_arithmetic_circuit_dag"
assert sp["division_allowed"] is False
assert sp["fanout_reuse_allowed"] is True
assert sp["size_counts_arithmetic_gates_only"] is True
assert sp["input_gates_counted"] is False
assert sp["dimension_threshold"] == 65536
assert sp["finite_bound_denominator"] == 144
assert sp["formula_target_count"] == 0 and sp["circuit_target_count"] == 3
...
: > "$scan_log"
if grep -nE '\b(sorry|admit)\b|^[[:space:]]*(axiom|unsafe)[[:space:]]' "$solution_file" >> "$scan_log"; then
  echo "prohibited solution declaration/placeholder detected" >&2
  cat "$scan_log" >&2
  exit 1
fi
...
echo "solution placeholder and unsafe/custom-axiom scan: clear" >> "$scan_log"
...

{
  ...
  echo "surface=OTP-C-PERMANENT-CIRCUIT"
...
axiom_file="MATHCERTPermanentCircuitReplayAxioms.lean"
{
  echo "import $solution_module"
  for theorem in "${theorem_names[@]}"; do echo "#print axioms $theorem"; done
  for theorem in "${witness_names[@]}"; do echo "#print axioms $theorem"; done
} > "$axiom_file"
lake env lean "$axiom_file" 2>&1 | tee "$output_dir/theorem-axioms.log"
rm -f "$axiom_file"

THEOREMS="$(printf '%s\n' "${theorem_names[@]}" "${witness_names[@]}")" AXIOM_LOG="$output_dir/theorem-axioms.log" AXIOM_REPORT="$output_dir/axiom-check.json" python3 - <<'PY'
import json, os, re
from pathlib import Path
...
for witness in "${witness_names[@]}"; do grep -Fq "${witness##*.}" "$solution_file"; done
...
record = {
  ...
  "record_type": "openai_ten_proofs_permanent_circuit_cert_replay_execution_evidence",
  "result_family": "OTP-C-PERMANENT",
  "surface_id": "OTP-C-PERMANENT-CIRCUIT",
  "execution": {
    ...
    "comparator": "accepted",
    "lean_default_kernel": "accepted",
    "nanoda_kernel": "accepted",
    "nonvacuity": "accepted",
    "unexpected_axioms": [],
...
(out / "replay-receipt.json").write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
PY

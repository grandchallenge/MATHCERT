...
def validation_errors(record=None, schema=None):
    r=load(RECORD) if record is None else record; s=load(SCHEMA) if schema is None else schema; routes=load(ROUTES); adjudication=load(ADJUDICATION); errors=[]
...
def main():
    ...
    print("validated A sphere-packing design-only restricted-output contract"); return 0

...
if [[ "$current_routes_blob" != "$registration_routes_blob" ]]; then
  echo "ERROR: certification route registry is neither the protected replay-stage snapshot nor the exact governed A registration successor: $current_routes_blob" >&2
  exit 1
fi
...
assert receipt['authority']['registered_route_registry_candidate_blob'] == 'b9bb0dc9e18856f50a88162df37c20c034327439'
...
print('A_REGISTRATION_SUCCESSOR_COMPATIBILITY=PASS')
...
replacement = 'echo "A registration successor validated separately; historical replay-stage route-registry assertion projected to its protected snapshot" >&2'

...
def validate_repository_guards() -> None:
    ...
    if "OTP-A-SPHERE-PACKING" in route_text or "MC-ROUTE-OTP-A-SPHERE-PACKING" in route_text:
        raise ValueError("sphere-packing route authority already present during intake-only operation")

def main() -> None:
    validate_record(load_record())
    validate_repository_guards()
    print("OTP-A-SPHERE-PACKING successor intake validation: PASS")

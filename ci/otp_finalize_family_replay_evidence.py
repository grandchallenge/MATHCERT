...
FAMILIES = {
    "OTP-H-GAPCVP": {"work_package": "governance/result_family_work_package_successors/OTP-H-GAPCVP-CERT-WP-001.json"},
    "OTP-B1-BINARY-CODES": {"work_package": "governance/result_family_work_package_successors/OTP-B1-BINARY-CODES-CERT-WP-001.json"},
    "OTP-B2-SPHERICAL-CODES": {"work_package": "governance/result_family_work_package_successors/OTP-B2-SPHERICAL-CODES-CERT-WP-001.json"},
}
...
def main()->int:
    ...
    wp=json.loads((ROOT/FAMILIES[family]["work_package"]).read_text(encoding="utf-8")); subject=wp["authority"]["official_subject"]; toolchain=wp["toolchain"]; scope=wp["target_scope"]; execution=wp["execution_contract"]
    ...
    text=comparator_log.read_text(encoding="utf-8",errors="replace")
    ...
    helper_paths={"landrun":Path(os.environ.get("COMPARATOR_LANDRUN_REAL","")),"lean4export":Path(os.environ.get("COMPARATOR_LEAN4EXPORT","")),"nanoda_bin":Path(os.environ.get("COMPARATOR_NANODA","")),"landrun_argv_adapter":Path(os.environ.get("COMPARATOR_LANDRUN",""))}
    helper_sha={}
    for key,path in helper_paths.items():
        ...
        helper_sha[key]=actual
    cfg=json.loads((UPSTREAM/subject["config_path"]).read_text(encoding="utf-8")); targets=scope["lean_theorems"]
    ...
    nonname="promise-nonvacuity-attestation.json" if family=="OTP-H-GAPCVP" else "nonvacuity-attestation.json"; write_json(out/nonname,{"state":scope["nonvacuity"]["state"],"protected_nonvacuity":scope["nonvacuity"],"new_nonvacuity_claim_added":False})
    ...
    summary=json.loads(summary_path.read_text(encoding="utf-8")); summary.update({"evidence_artifact_set_complete_for_runtime_replay":True,"exact_toolchain_identity":"verified","helper_binary_identity":"verified","independent_review_attestation":"pending_final_evidence_head_review"}); write_json(summary_path,summary)

...
def main() -> int:
    ...
    for required in (
        ...
        axiom_report,
        summary_path,
        out / "trust-boundary-scan.txt",
        out / "source-identity-report.txt",
    ):
    ...
    for key, rel in (
        ("config_blob", subject["config_path"]),
        ("challenge_blob", subject["challenge_path"]),
        ("solution_blob", subject["solution_path"]),
        ("lake_manifest_blob", "lake-manifest.json"),
        ("lean_toolchain_blob", "lean-toolchain"),
    ):
    ...
    if not same_unique_string_set(
        config.get("permitted_axioms"), execution["permitted_axioms"]
    ):
    ...
    write_json(
        out / "toolchain-identity-report.json",
        {
    ...
    write_json(
        out / "comparator-result.json",
        {"state": "accept", "marker": "Your solution is okay!"},
    )
    write_json(
        out / "lean-kernel-result.json",
        {"state": "accept", "marker": "Lean default kernel accepts the solution"},
    )
    write_json(
        out / "nanoda-result.json",
        {"state": "accept", "marker": "Nanoda kernel accepts the solution"},
    )
    write_json(
        out / "semantic-concordance-attestation.json",
        {
    ...
    nonvacuity_name = (
        "promise-nonvacuity-attestation.json"
        if family == "OTP-H-GAPCVP"
        else "nonvacuity-attestation.json"
    )
    write_json(
        out / nonvacuity_name,
        {
    ...
    if family == "OTP-B1-BINARY-CODES":
        write_json(
            out / "minimizer-attainment-attestation.json",
            {
                ...
                "evidence": [
                    x
                    for x in scope["nonvacuity"]["evidence"]
                    if "minim" in x.lower() or "mrrw" in x.lower()
                ],
        ...
        write_json(
            out / "hierarchy-domain-attestation.json",
            {
        ...
        write_json(
            out / "numerical-strengthening-attestation.json",
            {
    ...
    names = [
        ...
        if p.is_file() and p.name not in {"bundle.tar.gz", "bundle.sha256"}
    ]
    ...
    (out / "bundle.sha256").write_text(
        f"{sha256(out / 'bundle.tar.gz')}  bundle.tar.gz\n", encoding="utf-8"
    )

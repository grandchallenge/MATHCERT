...
def validate_record(record: dict, *, check_repository: bool = True) -> None:
    errors = sorted(Draft202012Validator(load(SCHEMA)).iter_errors(record), key=lambda e: list(e.path))
    if errors:
    ...
    require(record["contract"]["git_blob_sha1"] == PINS["governance/result_family_adjudication_contracts/OTP-J1-COMPACTNESS.json"], "contract pin drift")
    ...
    require(recipe["fresh_source_reacquisition_required"] is True and recipe["fresh_isolated_replay_required"] is True, "fresh evidence gate weakened")
    require(recipe["publication_must_be_descendant_of_authorized_input_head"] is True, "authorization ancestry gate removed")
    require(record["required_state"] == {
        "adjudication": None, "aggregate_adjudication": False, "aggregate_output": False, "cert_output": None,
        "mathematical_target_proved": False, "may_adjudicate": False, "may_promote_claim": False, "route_state": "submitted"
...
def self_test(base: dict) -> None:
    mutations = []
    mutations.append(lambda r: r["encoded_targets"].pop())
    mutations.append(lambda r: r["encoded_targets"].append("TwoDegenerateGraphs.not_erdos_146"))
    mutations.append(lambda r: r["current_source"].__setitem__("expected_sha256", "0" * 64))
    mutations.append(lambda r: r["current_source"].__setitem__("whole_document_equivalence_between_revisions", "established"))
    mutations.append(lambda r: r["contract"].__setitem__("contract_id", "OTHER"))
    mutations.append(lambda r: r["protected_evidence"].__setitem__("record_git_blob_sha1", "0" * 40))
    mutations.append(lambda r: r["decision_contract"].__setitem__("disposition_at_input_stage", "adjudication_clear_encoded_targets_only"))
    mutations.append(lambda r: r["decision_contract"]["admissible_dispositions"].append("qualified"))
    mutations.append(lambda r: r["execution_recipe"].__setitem__("authorization_must_name_contract_and_exact_head", False))
    mutations.append(lambda r: r["execution_recipe"].__setitem__("fresh_isolated_replay_required", False))
    mutations.append(lambda r: r["required_state"].__setitem__("route_state", "qualified"))
    mutations.append(lambda r: r["required_state"].__setitem__("cert_output", {}))
    mutations.append(lambda r: r["required_state"].__setitem__("mathematical_target_proved", True))
    mutations.append(lambda r: r["required_state"].__setitem__("aggregate_adjudication", True))
    mutations.append(lambda r: r["review_gate"].__setitem__("recorded_review", {"state": "APPROVED"}))
    mutations.append(lambda r: r.__setitem__("unexpected", True))
    for index, mutate in enumerate(mutations, 1):
        record = copy.deepcopy(base)
        mutate(record)
        try:
            validate_record(record, check_repository=False)
        except Exception:
            continue
        raise ValueError(f"mutation {index} was incorrectly accepted")
...
def main() -> int:
    ...
    try:
        ...
        print(f"Compactness adjudication input control failed: {exc}", file=sys.stderr)
        return 1
    print("validated frozen Compactness adjudication execution input; authorization and disposition remain absent")
    if args.self_test:
        print("Compactness adjudication input mutation suite passed")
    return 0

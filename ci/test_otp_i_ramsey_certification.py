...
import validate_otp_i_ramsey_certification as module


class RamseyCertificationTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_target_substitution_fails(self):
        cert = copy.deepcopy(self.values["certificate"]); cert["encoded_targets"][0] = "Ramsey.fake"
        self.assertTrue(any("target" in e for e in self.errors(certificate=cert)))

    def test_explicit_constant_reclassification_fails(self):
        cert = copy.deepcopy(self.values["certificate"]); cert["classifications"][1] = "source_verbatim"
    ...
    def test_specialist_gate_removal_fails(self):
        adj = copy.deepcopy(self.values["adjudication"]); adj["binding_gate"]["fresh_non_author_ramsey_combinatorics_Lean_specialist_approval_required"] = False

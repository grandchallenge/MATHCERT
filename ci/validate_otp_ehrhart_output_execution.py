...
SYNCHRONIZED_MAIN = "4bfcb6b10b233fcfc7992aa2f8a4eb40eac5dd66"
...
EXPECTED_BLOBS = {
    "candidate": "5ae75ed26f1cca54b8fbee6578199191f0d6dc93",
    "candidate_schema": "cd0515f4d2dbb6f3673cb7e1bf3cf001e92291c9",
    ...
    "routes_at_transition": "f4df18d612459af629615fdd36d67dad192a297a",
    "routes_at_reviewed_head": "75bc7e8da9c3ccf41cda8a733ca18742468a066d",
    "routes_on_synchronized_main": "cf876f43ae824f965a3aedf411671c110c380028",
}
...
ALLOWED_EXECUTION_FILES = {
    ...
    "ci/validate_otp_ehrhart_output_execution.py",
...
def ensure_history() -> None:
    ...
    for commit in (CORRECTION_MERGE, CONTENT_COMMIT, ROUTE_COMMIT, SYNCHRONIZED_MAIN):
...
def actual_git_receipt() -> dict[str, Any]:
    ...
    parent_result = run_git("rev-list", "--parents", "-n", "1", head)
    parent_tokens = parent_result.stdout.decode().strip().split() if parent_result.returncode == 0 else []
    review_base = parent_tokens[1] if len(parent_tokens) >= 3 else CORRECTION_MERGE
    return {
        "head": head,
        ...
        "synchronized_main_is_ancestor_of_head": is_ancestor(SYNCHRONIZED_MAIN, head) if head else False,
        ...
        "registry_blob_at_synchronized_main": git_object_blob(SYNCHRONIZED_MAIN, ROUTES_PATH),
        "registry_blob_at_head": git_object_blob(head, ROUTES_PATH) if head else None,
        "content_commit_files": changed_files(CONTENT_COMMIT),
        "route_commit_files": changed_files(ROUTE_COMMIT),
        "execution_changed_files": diff_files(review_base, head) if head else None,
    }
    ...
    *,
    ...
    if blobs is None:
        blobs = {
            ...
            "routes_at_reviewed_head": git_blob(ROUTES),
        }
    ...
    for key in ("candidate", "candidate_schema", "certificate", "transition", "future_schema", "routes_at_reviewed_head"):
    ...
    errors.extend(
        f"live certificate schema violation: {error.message}"
    ...
    if route_map.get("UC-001", {}).get("intake_status") != "qualified":
        errors.append("protected UC qualification was not retained")
    if route_map.get("UC-001", {}).get("cert_output", {}).get("digest") != "265c185d6b2b2970dc675729efa3fc4860f29204":
        errors.append("protected UC output identity drift")
    otp_outputs = [
    ...
    commits = candidate.get("execution_commits", {})
    ...
    if commits.get("route_registry_at_reviewed_head", {}).get("digest") != EXPECTED_BLOBS["routes_at_reviewed_head"]:
        errors.append("reviewed-head route registry binding drift")
    synchronization = candidate.get("protected_main_synchronization", {})
    if synchronization != {
        "merge_commit": SYNCHRONIZED_MAIN,
        "operation_id": "MC-UC-WP04-QUAL-001",
        "route_registry_blob": EXPECTED_BLOBS["routes_on_synchronized_main"],
        "effect": "retain_unrelated_protected_uc_qualification_without_changing_ehrhart_execution_commit_order",
    }:
        errors.append("protected-main synchronization authority drift")
    ...
    if not git_receipt.get("synchronized_main_is_ancestor_of_head"):
        errors.append("protected synchronized main is not an ancestor of reviewed merge head")
    ...
    if git_receipt.get("registry_blob_at_route_commit") != EXPECTED_BLOBS["routes_at_transition"]:
        errors.append("route-transition commit registry blob drift")
    if git_receipt.get("registry_blob_at_synchronized_main") != EXPECTED_BLOBS["routes_on_synchronized_main"]:
        errors.append("protected synchronized-main route registry drift")
    if git_receipt.get("registry_blob_at_head") != EXPECTED_BLOBS["routes_at_reviewed_head"]:
        errors.append("reviewed-head route registry blob drift")
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    receipt = actual_git_receipt()
    print(
        "validated OTP-F-EHRHART certificate-first execution, exact ancestor commit "
        f"{CONTENT_COMMIT}, later route transition {ROUTE_COMMIT}, synchronized main "
        f"{SYNCHRONIZED_MAIN}, and exact reviewed head {receipt.get('head')}"
    )
    return 0
